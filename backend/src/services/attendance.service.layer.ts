...
export const attendanceServiceLayer = {
  ...
  async markAttendance(studentId: string, sessionId: string, code: string) {
    ...
    if (!attendanceService.isSessionActive(session)) {
      throw new AppError(session.status === 'closed' ? 'Attendance session has been closed by the teacher' : 'Attendance session has expired', 400);
    }
    ...
    try {
      return await Attendance.create({ sessionId, courseId: session.courseId, studentId });
    } catch (error: any) {
      if (error.code === 11000) throw new AppError('You have already marked attendance for this session', 409);
      throw error;
    }
  },
  ...
  async getSessionStats(sessionId: string, userId: string, role: string) {
    assertId(sessionId);
    const session = await AttendanceSession.findById(sessionId).populate('courseId', 'name code').populate('teacherId', 'name email');
    if (!session) throw new AppError('Session not found', 404);
    const courseId = typeof session.courseId === 'object' && session.courseId ? session.courseId._id : session.courseId;
    ...
    return { session, presentCount: attendanceRecords.length, totalStudents, absentCount: Math.max(0, totalStudents - attendanceRecords.length), percentage: totalStudents ? ((attendanceRecords.length / totalStudents) * 100).toFixed(2) : '0', attendedStudents: attendanceRecords };
  },
  ...
  async getStudentAnalytics(studentId: string) {
    ...
    return {
      ...
      percentage: totalSessions ? Number(((records.length / totalSessions) * 100).toFixed(2)) : 0,
      courses: courses.map((course) => {
        const courseSessions = sessions.filter((s) => s.courseId.toString() === course._id.toString());
        const present = courseSessions.filter((s) => attended.has(s._id.toString())).length;
        return { courseId: course._id, name: course.name, code: course.code, totalSessions: courseSessions.length, attended: present, percentage: courseSessions.length ? Number(((present / courseSessions.length) * 100).toFixed(2)) : 0 };
      }),
    };
  },
  ...
  async getCourseAnalytics(courseId: string, teacherId: string) {
    ...
    return students.map((student) => {
      const present = attendanceRecords.filter((record) => record.studentId.toString() === student._id.toString()).length;
      const total = sessions.length;
      return { id: student._id, name: student.name, email: student.email, rollNumber: student.rollNumber, present, total, percentage: total ? Number(((present / total) * 100).toFixed(2)) : 0 };
    });
  },
